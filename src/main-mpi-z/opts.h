...
inline void UsageDual(){
	...
	std::cout << "EXAMPLES:\n"
			  << std::endl;
	std::cout << "mpirun -n 2 ./TiltRecZ-mpi --input ../../data/BBb/BBb_fin.mrc --output ../../data/BBb/BBb_BPT_y.mrc --tiltfile ../../data/BBb/BBb.rawtlt --geometry 0,0,0,300 --method BPT\n"
			  << std::endl;
}
		...
		case 'm':{
			...
			if ( strcmp(optarg,"BPT") && strcmp(optarg,"RP") ){
				...
				if(opts_->method == "SIRT"||opts_->method == "SART"){
					...
					EX_TRACE("Iteration Method: [--method/-m method(BPT,SIRT or SART) Iteration numbers,relax parameter]\n");
				}
